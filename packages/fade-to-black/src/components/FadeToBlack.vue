<template>
  <div class="fade-to-black container mx-auto">
    <div class="flex items-center flex-col py-8">
      <code class="block whitespace-pre overflow-x-scroll text-white py-4 h-60 overflow-scroll">
        {{ prettyResult || '' }}
      </code>
      <div>
        <button
          class="text-5xl bg-purple-500 hover:bg-purple-700 text-white font-bold py-8 px-12 rounded-xl"
          @click="solve"
        >
          Solve
        </button>
    ...
    <div class="h-96 overflow-scroll">
      <prism-editor v-model="model" class="editor" :highlight="highlighter" :line-numbers="true" />
    </div>
  </div>
</template>
...
for (let cloth1 of Object.entries(clothes).filter(([key, _]) => !/\d{1,2}\/\//g.test(key))) {
  const c = {}
  for (let clothType1 of Object.values(CLOTH_TYPES)) {
    for (let cloth2 of Object.entries(clothes).filter(
      ([key, value]) => !/\d{1,2}\/\//g.test(key) && value.type === clothType1 && key !== cloth1[0]
    )) {
      c[clothType1] = cloth2
  ...
  variables: outfits
    .map((o, i) => {
      return {
        ...
        appeal: Object.values(o).reduce((agg, value) => agg + clothes[value[0]].appeal, 0),
      }
    })
    .reduce((agg, value, i) => {
      agg[`outfit${i}`] = value
